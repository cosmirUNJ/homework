...
public class AIController implements InputController {
	...
	private static final int HYPOTETICAL_DIST = 6;
	...
	private void selectTarget() {
		//#region PUT YOUR CODE HERE
		//maksimum ship-nya
		//int idnya = ship.getId();
		int shipmax = fleet.size();
		float[] jarakantarship = new float[shipmax];
		//cek jarak ship ini dengan ship lain, kalo deket, target = ship itu
		//jarak dimasukin ke array, abis itu dicari yg paling pendek?
		for (int i=1;i<shipmax;i++){
			Vector2 ship2nya = fleet.get(i).getPosition();
			if(ship.getId() == i){
				//supaya gk targetin dirinya sendiri, dikasih range max
				jarakantarship[i] = Integer.MAX_VALUE;
			}
			else {
				jarakantarship[i] = manhattan(ship.getX(), ship.getY(), ship2nya.x, ship2nya.y);
			}
		}

		int hasiltargetnya = indexofshorttestt(jarakantarship);

		target = fleet.get(hasiltargetnya);
		//#endregion			
	}

	private int indexofshorttestt(float[] a){
		//cari bilangan terkecil
		//abis itu, retur index ke berapa?
		float kecil = Integer.MAX_VALUE;
		int indexnyah = 0;
	...
	private boolean canShootTargetFrom(int x, int y) {
		//#region PUT YOUR CODE HERE
		//int idship = ship.getId();
		//float idshipx = ship.getX();
		//float idshipy = ship.getY();

		//target = fleet.getPlayer();

		Vector2 playerpos = target.getPosition();

		float xplayerpos = Math.abs(x-playerpos.x);
		float yplayerpos = Math.abs(y-playerpos.y);

		float dist = manhattan(x,y,xplayerpos,yplayerpos);

		if(xplayerpos <= 1E-6 && dist <= HYPOTETICAL_DIST){
			return true;
		}
		else if(yplayerpos <= 1E-6 && dist <= HYPOTETICAL_DIST){
			return true;
		}
		else if(dist <= HYPOTETICAL_DIST && board.isPowerTileAt(x,y) && target.isAlive()){
			return true;
		}
	...
	private boolean canShootTarget() {
		//#region PUT YOUR CODE HERE
		if(ship.canFire() && canShootTargetFrom((int) ship.getX(),(int) ship.getY())){
			return true;
		}
